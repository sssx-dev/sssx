...
const POSTPROCESSING = fs.readFileSync(`${__dirname}/../patches/postProcessing.js`, {encoding: 'utf-8'}).split(SEPARATOR)
...
const getImportIndex = (lines:string[], options:Partial<Options>) => {
    ...
    for(let i=0;i<lines.length;i++){
        ...
        }else if(counter > 0){ // allow banner with comments first
            break;
        }
    }
...
export const wrapHydratableComponents = (text:string, path?:string) => {
    ...
    const cssImports = allImports.filter(line => line.includes(`.css`) && line.startsWith(`import "`))
    const cssFiles = cssImports
    .map(a => {
        let relativeURL = a
        .replace(`import "`,``)
        .replace(`";`,``)
