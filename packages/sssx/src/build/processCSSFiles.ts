...
const copySingleCSSFile = async (cssPath:string, setFilesMap:(k:string,v:string)=>void, dstBase:string[] = [OUTDIR]) => {
    ...
    dstBase.map(dst => {
        const dstCssPath = cssPath
        .replace(config.sourceRoot, dst)
        .replace(base, newFilename)
...
export const processCSSFiles = async (entryPointsCSS:string[], setFilesMap:(k:string,v:string)=>void,) => {
