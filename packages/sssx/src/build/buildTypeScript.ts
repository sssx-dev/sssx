...
export const buildTypeScript = async (entryPoints:string[], setFilesMap:(k:string,v:string)=>void) => {
    ...
    await Promise.all(result.outputFiles.map(async (output, index) => {
